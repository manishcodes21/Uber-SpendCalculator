...
`,
  ...
  try {
    ...
    const tripDetailsPromises = allTrips.map((trip) =>
      fetchTripDetails( csrfToken, trip.uuid)
    );
...
async function fetchTripDetails( csrfToken, tripUUID) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "sendTripData") {
     fetchAllTrips().then((response) => {
            console.log("Background Script - Received Trips:", response); // Should display data
            sendResponse({ success: true, trips: response });
         }
     )
    return true;
  }
